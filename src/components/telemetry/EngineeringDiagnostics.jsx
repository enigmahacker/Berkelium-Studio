...
export default function EngineeringDiagnostics() {
  ...
  const isReady = simulationState === 'PROJECT_READY' && !telemetry;

  // Red warnings strictly driven by solver flow_separation flag
  const compMatrix = telemetry?.componentStatusMatrix || [];

  const getStatusBadge = (status) => {
    // Only render critical red badge if solver reported flow_separation
    if ((status === 'STALLED' || status === 'SEPARATED' || status === 'EXPANDED_SEPARATED_WAKE') && telemetry?.flow_separation) {
      return (
  ...
  return (
    <div className="w-full h-full flex flex-col bg-zinc-950 text-zinc-100 overflow-hidden text-xs select-none">
      ...
      <div className="flex-1 overflow-y-auto p-3.5 space-y-4 custom-scrollbar">
        {/* If NO_PROJECT */}
        {isNoProject ? (
          <div className="p-6 rounded-xl bg-zinc-900/50 border border-zinc-800/80 text-center space-y-3">
            <div className="w-12 h-12 rounded-xl bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-500 mx-auto">
              <FolderX size={20} className="text-amber-500" />
            </div>
            <div>
              <h4 className="font-bold text-zinc-200 text-sm">NO SIMULATION PROJECT LOADED</h4>
              <p className="text-zinc-500 text-xs mt-1 max-w-xs mx-auto">
                Select a valid project directory to initialize the Reduced-Order Aerodynamic Model.
              </p>
            </div>
            <button
              onClick={openProjectLauncher}
              className="px-3.5 py-1.5 rounded bg-orange-600 hover:bg-orange-500 text-white font-semibold text-xs transition-colors cursor-pointer"
            >
          ...
          <div className="p-6 rounded-xl bg-zinc-900/50 border border-zinc-800/80 text-center space-y-3">
            <div className="w-12 h-12 rounded-xl bg-blue-950/50 border border-blue-500/30 flex items-center justify-center text-blue-400 mx-auto">
              <Activity size={20} />
            </div>
            <div>
              <h4 className="font-bold text-zinc-200 text-sm">PROJECT READY</h4>
              <p className="text-zinc-400 text-xs mt-1">
                Source: <span className="font-mono text-orange-300">{project?.sourceFile}</span>
              </p>
              <p className="text-zinc-500 text-[11px] mt-0.5">
                Click Run to execute the Reduced-Order Aerodynamic Model and calculate telemetry.
              </p>
            </div>
            <button
              onClick={runSimulation}
              className="px-4 py-2 rounded bg-orange-600 hover:bg-orange-500 text-white font-bold text-xs transition-colors shadow flex items-center justify-center gap-1.5 mx-auto cursor-pointer"
            >
              <Play size={13} className="fill-current" />
              <span>Run Aerodynamic Simulation</span>
            </button>
          </div>
        ) : (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="p-2.5 rounded bg-zinc-900/60 border border-zinc-800 flex flex-col justify-between">
                <div className="flex items-center justify-between text-zinc-400 text-[10px] font-mono">
                  ...
                  <span className="text-orange-400 font-bold">Cd {telemetry?.cd?.toFixed(3) || '0.320'}</span>
                </div>
                ...
                <div className="text-[10px] text-zinc-500 font-mono mt-0.5">
                  Area: {telemetry?.frontalArea || 2.74} m²
                </div>
              </div>
              ...
              <div className="p-2.5 rounded bg-zinc-900/60 border border-zinc-800 flex flex-col justify-between">
                <div className="flex items-center justify-between text-zinc-400 text-[10px] font-mono">
                  ...
                  <span className="text-cyan-400 font-bold">Cl {telemetry?.cl?.toFixed(3) || '0.200'}</span>
                </div>
                <div className="mt-1 flex items-baseline gap-1">
              ...
              <div className="p-2.5 rounded bg-zinc-900/60 border border-zinc-800 flex flex-col justify-between">
                ...
                <div className="text-[10px] text-zinc-500 font-mono mt-0.5">
                  Re: {(telemetry?.reynoldsNo ? (telemetry.reynoldsNo / 1e6).toFixed(2) : '14.59')} × 10⁶
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5">
                {compMatrix.map((item) => {
                  const isStall = item.severity === 'critical' && telemetry?.flow_separation;
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className={`text-[10px] leading-tight ${
                        isStall ? 'text-red-300 font-medium' : isWarning ? 'text-amber-300' : 'text-zinc-500'
                      }`}>
                        {item.description}
                      </p>
                    </div>
